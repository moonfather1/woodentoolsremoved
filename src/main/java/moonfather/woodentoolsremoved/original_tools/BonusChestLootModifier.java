...
public class BonusChestLootModifier extends LootModifier
{
    ...
    {
        super(conditionsIn, 5);
    }
    public BonusChestLootModifier(LootItemCondition[] conditionsIn, int priority)
    {
    ...
    {
        if (! context.getQueriedLootTableId().equals(BuiltInLootTables.SPAWN_BONUS_CHEST.identifier()))
        {
        ...
        {
            if (generatedLoot.get(i).is(ItemTags.PICKAXES))
            {
                generatedLoot.set(i, new ItemStack(RegistryManager.ItemHatchet.get()));
            }
            else if (generatedLoot.get(i).is(ItemTags.PICKAXES))
            {
    ...
    public static final Supplier<MapCodec<BonusChestLootModifier>> CODEC = Suppliers.memoize(() ->
