...
public class FirestarterItem extends FlintAndSteelItem
{
    ...
    {
        super((new Properties()).durability(6).setNoRepair().tab(CreativeModeTab.TAB_TOOLS));
    }
    ...
    private HitResult calculateHitResult(Player player)
    {
        return ProjectileUtil.getHitResult(
                player, entity -> ! entity.isSpectator() && entity.isPickable()      // reduced range moved to useOn
        );
    }
        ...
        if (ticksRemaining >= 0 && entity instanceof Player player)
        {
            ...
            if (hitresult instanceof BlockHitResult blockhitresult && hitresult.getType() == HitResult.Type.BLOCK)
            {
                if (blockhitresult.distanceTo(player) < Constants.GAMEPLAY_FIRESTARTER_RANGE)
                {
                    ...
                    if (doParticles)
                    {
                        Vec3 forward = player.position().vectorTo(Vec3.atCenterOf(blockhitresult.getBlockPos())).normalize();
                        for (int i = 0; i < 3; i++)
                        {
    ...
    private static final Component ERROR_TOO_FAR = new TranslatableComponent("item.woodentoolsremoved.firestarter.error").withStyle(Style.EMPTY.withColor(Constants.COLOR_MESSAGE_GRAY));
    ...
    private static final Component TooltipForFirestarterLine1 = new TranslatableComponent("item.woodentoolsremoved.firestarter.tooltip1").withStyle(Style.EMPTY.withColor(Constants.COLOR_GRAY_TOOLTIPS));
    private static final Component TooltipForFirestarterLine2 = new TranslatableComponent("item.woodentoolsremoved.firestarter.tooltip2").withStyle(Style.EMPTY.withColor(Constants.COLOR_GRAY_TOOLTIPS));
}