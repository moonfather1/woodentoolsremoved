...
public class EventForPunchingLogs
{
	...
	private static final TagKey<Item> TAG_TOOLS = ItemTags.create(new ResourceLocation("forge", "tools"));
	...
	public static void OnBreakSpeed(PlayerEvent.BreakSpeed event)
	{
		...
		{
			if (ToolResolver.isWoodenAxe(event.getPlayer().getMainHandItem())
				|| ToolResolver.isWoodenPickaxe(event.getPlayer().getMainHandItem())
				|| OptionsHolder.IsResolvedModeHard() && ToolResolver.isFlintPickaxe(event.getPlayer().getMainHandItem()))
			{
            ...
            if (usingTetra && TetraSupport.IsWoodenTetraTool(event.getPlayer().getMainHandItem()))
            {
				if (ShouldShowMessage(event.getPlayer()))
				{
		...
		if (event.getState().is(BlockTags.LOGS) && ! event.getPlayer().getMainHandItem().isCorrectToolForDrops(event.getState()))
		{
			...
			if (! event.getPlayer().getMainHandItem().isEmpty() && event.getPlayer().getMainHandItem().is(TAG_TOOLS))
			{
				event.setNewSpeed(event.getOriginalSpeed() / 8);
				event.setCanceled(false);
				return;
			}
			if (! event.getPlayer().level.isClientSide() && ShouldGiveAdvancement(event.getPlayer()))
			{
				if (usingMultimine && (event.getPos() == null || (event.getPos().getX() == 0 && event.getPos().getZ() == 0) || ShouldAbortMultiMine(event.getPlayer(), event.getPos())))
				{
			...
			if (! event.getPlayer().level.isClientSide() && ShouldShowMessage(event.getPlayer()))
			{
				if (usingMultimine && (event.getPos() == null || (event.getPos().getX() == 0 && event.getPos().getZ() == 0) || ShouldAbortMultiMine(event.getPlayer(), event.getPos())))
				{
					return; // this mod keeps asking about break speed after we stop hitting the block.
				...
				{
					event.getPlayer().hurt(DamageSource.FLY_INTO_WALL, 1);
					int m = event.getPlayer().level.getRandom().nextInt(handHurtsMessages.length);
					event.getPlayer().displayClientMessage(handHurtsMessages[m], true);
				}
				else
				{
					int m = event.getPlayer().level.getRandom().nextInt(handNoEffectMessages.length);
